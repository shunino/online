				...
				<div class='item_group'>
					<div v-for="item in list" class='item' @click='goNewsDetail(item.id);'>
						...
						<div class="item_right">
							...
							<div class='time'><img src="../../assets/icon-clock.png">{{item.updateDate.substring(0,10)}}<span class='author'><img src="../../assets/news.png" height="12" width="12" alt="">&nbsp;<span v-if='item.author'>{{item.author}}</span><span v-else>绿城新闻</span></span></div>
						</div>
					</div>
				</div>
				          ...
				          if (data && data.admNews) {
			                ...
			                if(data.total <= 10*this.params.pageNo){
			                	// _this.list = data.admNews;
					              ...
					              $(".mint-loadmore-bottom .mint-loadmore-text").text("无更多快讯！");
